        ...
        if (Input.GetKeyDown(KeyCode.Mouse1))
        {
            ...
            if (Physics.Raycast(ray, out RaycastHit raycastHit, float.MaxValue, layerMask))
            {
                ...
                foreach (GameObject unit in selectedUnits)
                {
                    ...
                    unit.GetComponent<MoveSubV11>().targetPosition = targetPointsInLine[minDistanceIndex];
                    ...
                    print("Target position = " + unit.GetComponent<MoveSubV11>().targetPosition + " for " + unit.name);
                    Debug.DrawLine(unit.transform.position, unit.GetComponent<MoveSubV11>().targetPosition, Color.black, 10.0f);
                }
