...
public class RTSSelection : MonoBehaviour
{
    ...
    private void ConfirmSingleSelection()
    {
        ...
        for (int i = 0; i < hits.Length; ++i)
        {
            ...
            print(selectable.gameObject.name + ", team = " + selectable.gameObject.GetComponent<MoveSubStandart>().team);
    ...
    private void FinalizeSelection()
    {
        ...
        for (int i = 0; i < toBeSelected.Count; ++i)
        {
            ...
            if (selectedCounts.ContainsKey(unitType))
                selectedCounts[unitType]++;
            else
                selectedCounts.Add(unitType, 1);
        }
        print("selectedCounts.Count = " + selectedCounts.Count);
        gameManager.SetIcons(selectedCounts);
